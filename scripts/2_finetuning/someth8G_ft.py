...
tracking_dir = "logs/fine_tuning_tracking"
...
def register_hooks(model):
    """Registers forward hooks for activation & attention capture."""
    for layer_idx in tracked_layers:
        # Dynamically find the transformer layers
        if hasattr(model, "transformer") and hasattr(model.transformer, "h"):
            transformer_layer = model.transformer.h[layer_idx]
        elif hasattr(model, "vla") and hasattr(model.vla, "encoder"):
            transformer_layer = model.vla.encoder.layers[layer_idx]
        else:
...
def save_tensor(data, filename):
    ...
    if isinstance(data, dict):
        ...
        data = data.cpu().half() if data.dtype != torch.float32 else data.cpu()  # Keep loss in float32
...
def capture_weight_changes(model, step):
    """Capture LoRA weight changes, storing only trainable parameters."""
    lora_weights = {
        name: param.detach().cpu()
        for name, param in model.named_parameters()
        if "lora" in name and param.requires_grad  # Track only updated LoRA weights
    }
...
def capture_feature_representations(model, step):
    """Capture feature representations (hidden states) from the encoder layers."""
    feature_representations = {
        layer: activation_store.get(f"layer_{layer}", torch.zeros(1)) for layer in tracked_layers
    }
...
def compute_hessian_metric(model, loss):
    """Computes Hessian-based sharpness using loss gradients."""
    loss = loss.mean()  # Ensures correct computation on batch inputs
    ...
    try:
        ...
        hessian_norm = torch.tensor(-1.0)  # Placeholder value for tracking

    return hessian_norm.detach().cpu()


def capture_hessian_based_metrics(model, loss, step):
    """Capture Hessian-based sharpness metrics using loss function."""
...
def finetune(cfg: FinetuneConfig) -> None:
    ...
    # Register forward hooks for activation & attention tracking
    register_hooks(vla)
